...
function sockMerchant(n, arr) {
    //create obj that holds pairs.
    let obj ={};
    //create return var to hol # of pairs
    let pairCount = 0;


    //loop through the arr and add each element to the object.
    for (let i = 0;i < arr.length; i++){
        //if obj[arr[i]] === undefined, set it to the object with value of 1

        if (obj[arr[i]] === undefined){
            obj[arr[i]] = 1;
        } else if (obj[arr[i]] === 1){
            //if obj[arr[i]] === 1 then ++pairCount, reset obj[arr[i]] === undeifined
            pairCount++;
            obj[arr[i]] = undefined;
        }
    }
...
console.log(sockMerchant(10,[1,2,1,4,2,3,3,5,6,7]));